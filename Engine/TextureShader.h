...
#ifndef _TEXTURESHADER_H // Si on a jamais appele cet header
#define _TEXTURESHADER_H // On signifie qu'on l'appel
...
{
	public:
		CTextureShader(); // Constructeur
		CTextureShader(const CTextureShader&); // Constructeur de copie
		~CTextureShader(); // Destructeur

		bool Init(ID3D10Device*, HWND); // Initialisation
		void Shutdown(); // Destruction propre
		void Render(ID3D10Device*, int, D3DXMATRIX, D3DXMATRIX, D3DXMATRIX, ID3D10ShaderResourceView*); // Affiche à l'ecran les donnees passees au shader
